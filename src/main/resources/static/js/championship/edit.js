...
var app = new Vue({
		...
		fetch('/api/member/all')
			...
			.then(data => {		// JSONデータ
				...
				this.playerDetail = {
					...
					useAllBrawlers: 0,
					useLongRange: 0,
					useLongRangeSupHeavy: 0,
					useMidRange: 0,
					useMidRangeSupHeavy: 0,
					useHeavyWeight: 0,
					useSemiHeavyWeight: 0,
					useThrower: 0
				}
			...
			fetch('/api/championship/show/' + this.championshipId)
				...
				.then(data => {
					// 基本情報をフォームに受け渡し
					const convertedDate = new Date(new Date(data.date) - 9 * 60 * 60 * 1000);	// DB取得値からのズレ9時間を補正
	...
	methods: {
		...
		// ドーナツチャート描画（大会新規作成、メンバー一覧画面とソース重複状態）
